...
def run_no_simulation(x, fuzzing_content, fuzzing_arguments, sim_specific_arguments, dt_arguments, launch_server, counter, port):
    from no_simulation_dataset_script.no_simulation_objectives_and_bugs import check_bug, classify_bug_type

    if sim_specific_arguments.oracle is None:
        from sklearn.neighbors import KDTree

        oracle_keys = sim_specific_arguments.df[fuzzing_content.labels].to_numpy()
        oracle = KDTree(oracle_keys, leaf_size=1)
    else:
    ...
    # print('dist', dist)
    # print('inds', inds)
    # print('oracle_keys[inds]', oracle_keys[inds])
    # print('oracle_values[inds]', oracle_values[inds])
